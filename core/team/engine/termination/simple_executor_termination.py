...
class PMCASimpleExecutorTermination:
    ...
    def _functional_termination(
        ...
        return (
            PMCARoutingMessages.SIMPLE_EXECUTOR_SUCCESS.value in messages[-1].content  # type: ignore
            or PMCARoutingMessages.SIMPLE_EXECUTOR_FAILURE.value in messages[-1].content  # type: ignore
        )
    ...
    def termination(self):
        ...
        return reduce(operator.or_, active) & SourceMatchTermination(
            [PMCACoreAssistants.USER_PROXY.value]
        )
