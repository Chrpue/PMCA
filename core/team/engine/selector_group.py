...
class PMCASelectorGroup:
    ...
    def _termination_condition(self):
        ...
        # 监听来自 Planner 的完成信号
        text_mention_termination = TextMentionTermination(PMCARoutingMessages.TASK_TERMINATE.value, PMCACoreAssistants.ORCHESTRATOR,)

        # 监听来自 UserProxy 的信号
        user_mention_termination = TextMentionTermination(PMCARoutingMessages.SIGNAL_CANCEL.value, PMCACoreAssistants.USER_PROXY)

        return text_mention_termination | user_mention_termination | max_messages_termination

    def _build_orchestrator(self):
        try:
            orchestrator = self._ctx.assistant_factory.create_agent(biz_type=PMCACoreAssistants.ORCHESTRATOR)
