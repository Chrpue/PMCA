...
def build_functional_workbenches(ctx: PMCATaskContext) -> List[Workbench]:
    ...
    统一构造成 McpWorkbench 列表（SSE）。
    """
    sse_map = ctx.task_env.get_functional_servers()
    return [McpWorkbench(server_params=params) for params in sse_map.values()]
...
def merge_functional_workbenches(
    ...
    """
    if PMCACoreAssistants.is_core_assistant(assistant_name):
        return params
